...
- Name: 10-controladores-padre.spec.js >> CP — Controladores de datos extraídos (js/controladores-padre.js) >> CP-3. El total de controladores registrados es >= 11 (incluye los 4 extraídos)
- Location: tests\e2e\10-controladores-padre.spec.js:71:3
...
> 76  |     expect(setSize).toBeGreaterThanOrEqual(11);
  ...
  155 |   });
  156 | 
  157 |   test('CP-8. SOLICITAR_DATOS_PARADAS → handler registrado y no lanza', async ({ page }) => {
  158 |     const result = await enviarSolicitudYEsperarRespuesta(
  159 |       page,
  160 |       'NAVEGACION.SOLICITAR_DATOS_PARADAS',
  161 |       'NAVEGACION.RESPUESTA_DATOS_PARADAS'
  162 |     );
  163 |     expect(result.ok || result.razon.includes('timeout')).toBe(true);
  164 |   });
  165 | });
  166 | 
```