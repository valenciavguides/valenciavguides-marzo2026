...
globalThis.runTest = async function(){
  ...
  if (!hasActivar) {
    append('[test] ERROR: globalThis.activarGPS no está disponible en iframe después de esperar');
  ...
  // Poll globalThis.estadoPadre.gps for expected fallback
  const timeoutMs = 5000;
  const start = Date.now();
  let passed = false;
  while (Date.now() - start < timeoutMs) {
    const estado = win.window && win.globalThis.estadoPadre && win.globalThis.estadoPadre.gps;
