...
| `AUDIO_AUTOMATICO` | ¿Se reproduce el audio automáticamente al llegar a la parada? | No — verificado en código (`autoplay:false` siempre, en `codigo-padre.html` y `audio-hijo3.html`): al llegar, el audio se carga y queda listo en los controles del padre, pero el usuario tiene que pulsar play. | RESUELTA | |
