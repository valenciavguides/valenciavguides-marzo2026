...
**Radio de activación de parada: 20m → 10m → 15m.** El usuario reportó, aparte del bug de la flecha, que en aventuras con dos paradas del mismo monumento muy próximas (parada-tramo-parada con poca distancia real entre ellas), el radio de 20m bastaba para que, al completar la primera parada, el usuario ya estuviera también dentro del radio de la segunda — el tramo intermedio se daba por completado sin caminarlo de verdad. Bajado a 10m como primer ajuste. Cuatro sitios cambiados, todos el mismo valor: `RADIO_PARADA` y `rangoMaximo` (×2, dos funciones) en `coordenadas-hijo2.html`, y el círculo naranja visual (`circuloActivacion`) en `js/funciones-mapa.js`. Documentado en detalle, con la razón completa, en GUIA-COMPLETA §25.5 ("Por qué 15 m y no 20 m" / "Por qué 15 m y no 10 m").

**Actualización posterior:** en uso real de campo, 10m resultó demasiado estricto — el usuario podía tener que esperar un buen rato parado exactamente encima del punto antes de que el círculo confirmara la llegada, dando la sensación de que la detección no funcionaba. Subido a 15m en los mismos cuatro sitios, como término medio entre la precisión que motivó bajarlo de 20 y un margen razonable para el ruido de GPS real.

**Si en el terreno el tramo corto sigue completándose demasiado rápido incluso con el radio de parada ya en 15m, este cambio probablemente NO es la causa — hay que mirar en otro sitio, ya identificado:**
...
1. **Tamaño/forma/borde unificados con `#selector-tipo-mapa`.** Antes `#brujula-modo` era circular sin borde y `#selector-tipo-mapa` cuadrado (`border-radius:4px`) con borde naranja `#FF8C00` — inconsistente con el resto de botones flotantes de la app (chat, badges del mapa), que son circulares con ese mismo borde. Los dos pasan a compartir exactamente el mismo estilo (circular, borde naranja). El tamaño del botón principal subió en tres pasos, cada uno tras ver el anterior en el navegador y pedir un poco más: `clamp(36px,9.8vmin,52px)` → `clamp(40px,11vmin,58px)` → `clamp(46px,12.6vmin,67px)` → `clamp(46.92px,12.852vmin,68.34px)` (+2% sobre el tercero, con las opciones del desplegable sin tocar). El desplegable subió solo en los dos primeros pasos y se queda fijo desde entonces: `clamp(29px,8.4vmin,43px)` → `clamp(32px,9.2vmin,47px)` → `clamp(37px,10.6vmin,54px)` (tamaño actual). Verificado con Playwright en cada paso, en un viewport de móvil real (390×844): con el tamaño final, el desplegable ya expandido deja un margen de ~67px hasta el borde inferior de `#fondo-blanco` — lejos de solaparlo.
