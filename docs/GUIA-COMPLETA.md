...
- `CACHE_VERSION` se actualiza en cada commit (valor actual: `'v-video-intro-jul08'`). El sistema de auto-generación por SHA-256 vía `tools/build-sw.js` está descrito en los comentarios del SW pero el archivo no existe todavía.
...
const CACHE_VERSION = 'v-video-intro-jul08'; // ← cambiar a un identificador de la versión (p.ej. 'v-1.0.0')
...
```js
const CACHE_VERSION = 'v-video-intro-jul08';
```
